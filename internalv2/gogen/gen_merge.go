...
func generateFuncMergeDirtyToPb(fw *gen.FileWriter, msg gen.SyncMsgOrEnumDef) {
	...
	for _, field := range msg.MsgFields {
		...
		if field.IsMap() {
			...
			fw.PLF("if r.%s != nil {", field.CapitalName)
			fw.PLF("for i := len(r.%s)-1 ; i >= 0 ;i-- {", field.CapitalName)
			fw.PLF("v := r.%s[i]", field.CapitalName)
			fw.PLF("k := v.Get%s()", field.MsgOrEnumRef.MsgKey.CapitalName)
			fw.PLF("if x.%s.ContainDeleted(k) {", field.Name)
			fw.PLF("r.%s = append(r.%s[:i], r.%s[i+1:]...)", field.CapitalName, field.CapitalName, field.CapitalName)
			fw.PLF("continue")
			fw.PLF("}")
			...
			fw.PLF("if r.%s[i] == nil{", field.CapitalName)
			fw.PLF("r.%s[i] = &%s{}", field.CapitalName, field.MsgOrEnumRef.Name)
			fw.PLF("}")
			fw.PLF("tmp.MergeDirtyToPb(r.%s[i])", field.CapitalName)

			fw.PLF("}")
			fw.PLF("}")
			fw.PLF("} else {")
			fw.PLF("r.%s = make([]*%s,0)", field.CapitalName, field.MsgOrEnumRef.Name)
			fw.PLF("}")

			fw.PLF("for k := range x.%s.Dirtied() {", field.Name)
			fw.PLF("if !slices.Contains(updated,k) {")
			fw.PLF("tmp := x.%s.Get(k)", field.Name)
			fw.PLF("v := &%s{}", field.MsgOrEnumRef.Name)
			fw.PLF("tmp.MergeDirtyToPb(v)")
			fw.PLF("v.Set%s(tmp.Get%s())", field.MsgOrEnumRef.MsgKey.CapitalName, field.MsgOrEnumRef.MsgKey.CapitalName)
			fw.PLF("r.%s = append(r.%s,v)", field.CapitalName, field.CapitalName)
			fw.PLF("}")
			fw.PLF("}")
...
func generateFuncMergeDirtyToBytes(fw *gen.FileWriter, msg gen.SyncMsgOrEnumDef) {
	...
	for _, field := range msg.MsgFields {
		...
		if field.IsMap() {
			...
			fw.PLF("return true")
			fw.PLF("}")
			fw.PLF("vb := v.MergeDirtyToBytes()")
			fw.PLF("buf = protowire.AppendTag(buf, %d, %v)", field.Number, syncdep.FieldTypeToWireType(field))
			fw.PLF("buf = syncdep.AppendFieldValue(buf, vb)")
			fw.PLF("return true")
			fw.PLF("})")
			fw.PL("}")
		}
...
func generateFuncMergeDirtyFromBytes(fw *gen.FileWriter, msg gen.SyncMsgOrEnumDef) {
	...
	for _, field := range msg.MsgFields {
		...
		if field.IsMap() {
			...
			fw.PLF("tmp.MergeDirtyFromBytes(rawF.Value.([]byte))")
			fw.PLF("k := tmp.Get%s()", field.MsgOrEnumRef.MsgKey.CapitalName)
			fw.PLF("v := x.Get%s().Get(k)", field.CapitalName)
			fw.PLF("if v == nil {")
			fw.PLF("x.Get%s().Put(k,tmp)", field.CapitalName)
			fw.PLF("} else {")
			fw.PLF("v.MergeDirtyFromBytes(rawF.Value.([]byte))")
			fw.PLF("}")
		}
	}
	fw.PLF("}")
...
func generateFuncMergeDirtyFromPb(fw *gen.FileWriter, msg gen.SyncMsgOrEnumDef) {
	...
	for _, field := range msg.MsgFields {
		...
		if field.IsMap() {
			...
			fw.PLF("for _,v := range r.%s {", field.CapitalName)
			fw.PLF("var tmp = x.Get%s().Get(v.Get%s())", field.CapitalName, field.MsgOrEnumRef.MsgKey.CapitalName)
			fw.PLF("if tmp == nil {")
			fw.PLF("tmp = New%s()", field.MsgOrEnumRef.SyncName)
			fw.PLF("tmp.MergeDirtyFromPb(v)")
			fw.PLF("x.Get%s().Put(v.Get%s(),tmp)", field.CapitalName, field.MsgOrEnumRef.MsgKey.CapitalName)
			fw.PLF("} else {")
			fw.PLF("tmp.MergeDirtyFromPb(v)")
			fw.PLF("}")
			fw.PLF("}")
